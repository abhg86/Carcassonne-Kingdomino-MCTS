...
class NNetWrapper():
    ...
    def predict(self, numpy_state):
        """
        numpy_state: np array with board, meeples, next tile and current player
        """
        # timing
        start = time.time()
        ...
        # board = board.view(1, self.board_x, self.board_y)         # No need, already 1D
        self.nnet.eval()
        with torch.no_grad():
...
class Carcassonne_NNet(nn.Module):
    def __init__(self, args, player_nb, board_size=BOARD_SIZE):
        # game params
        self.tensor_len = board_size*board_size * (5*4 + 5) + 5*4 + 5*player_nb
        self.action_size = board_size*board_size*5 +1
        self.args = args
